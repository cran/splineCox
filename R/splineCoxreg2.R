...
#'   \item \code{loglik}: A named vector containing the log-likelihood (\code{LogLikelihood}), Akaike Information Criterion (\code{AIC}), and Bayesian Information Criterion (\code{BIC}) for the best-fitting model.
#'   \item \code{other_models}: A data frame containing the log-likelihood (\code{LogLikelihood}) for all other evaluated models, with model names as row names.
#'   \item \code{plot}: A baseline hazard function plot for the best-fitting model (if \code{plot = TRUE}).
#' }
#' @examples
#' # Example data
#' library(joint.Cox)
